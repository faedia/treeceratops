...
template <class T, class Allocator, class Ttree> class tree_iterator
{
  ...
  public:
    ...
    template <class T1, class Allocator1, class Ttree1> friend class tree_iterator;
...
template <class T, class Allocator = std::allocator<node_type_<T>>> class tree
{
  public:
    ...
    using const_iterator = const tree_iterator<T, Allocator, const tree<T, Allocator>>;
    friend iterator;

    // Element Access
    T &at(node_id id)
    {
        return data_.at(id).data;
    }
    T &operator[](node_id id)
    {
    ...
    void push_child(const iterator &pos, const T &value)
    {
        ...
        {
            data_.push_back({pos.node_, data_[pos.node_].children.back(), {}, value, {}});
